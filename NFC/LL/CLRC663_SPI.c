 ...
 * @details     由于 CLRC663 采用管线式读时序：
 *              1. 发送地址字节（LSB = 1 代表读），此时从机返回空闲/无效字节
 *              2. 发送一个虚拟字节（0x00），此时从机才会把刚刚解码的地址数据返回
 * @param       reg 寄存器物理地址
 * @return      读取到的数值
 */
uint8_t CLRC663_ReadReg(uint8_t reg) {
    /* 地址字节格式：Bit 7..1 存放寄存器地址高 7 位，LSB (Bit 0) 为 1 代表读操作 */
    uint8_t addr_byte = (reg << 1) | 0x01;
    uint8_t dummy = 0x00;
    uint8_t rx_data = 0;
    ...
    /* 步骤 1：发送寄存器读地址字节 */
    HAL_SPI_Transmit(&hspi1, &addr_byte, 1, 100);
    /* 步骤 2：发送 Dummy 字节获取刚才所读寄存器的值 */
    HAL_SPI_TransmitReceive(&hspi1, &dummy, &rx_data, 1, 100);
    CLRC663_NSS_HIGH();
